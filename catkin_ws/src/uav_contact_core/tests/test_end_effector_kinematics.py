...
def test_compute_twist_generates_xy_velocity_from_trajectory_velocity():
    ...
    controller.set_phase_enabled(True)
    twist = controller.compute_twist()

    assert abs(twist.linear.x) <= 0.4
    assert abs(twist.linear.y) <= 0.4
    assert twist.linear.x != 0.0 or twist.linear.y != 0.0


def test_compute_twist_returns_zero_when_phase_disabled_with_trajectory():
    module = _load_end_effector_module()
    controller = module.EndEffectorTwistController(link_length=0.7835, vel_factor=2.0, max_xy_speed=0.4)
...
def test_compute_twist_returns_zero_after_phase_disabled_again():
    module = _load_end_effector_module()
    controller = module.EndEffectorTwistController(link_length=0.7835, vel_factor=2.0, max_xy_speed=0.4)
    ...
    controller.update_trajectory_reference(_Traj())
    controller.set_phase_enabled(True)
    enabled_twist = controller.compute_twist()
    controller.set_phase_enabled(False)
    disabled_twist = controller.compute_twist()

    assert enabled_twist.linear.x != 0.0 or enabled_twist.linear.y != 0.0
    assert disabled_twist.linear.x == 0.0
    assert disabled_twist.linear.y == 0.0
